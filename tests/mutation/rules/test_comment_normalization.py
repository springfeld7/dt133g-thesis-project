...
class TestCommentNormalizationCoreBehavior:
    ...
    def test_normalizes_line_comment(self, length):
    ...
    @pytest.mark.parametrize("length", [1, 4, 9])
    def test_normalizes_block_comment(self, length):
    ...
    @pytest.mark.parametrize("length", [1, 4, 9])
    def test_normalizes_python_docstring(self, length):
    ...
    def test_multiple_comments(self, multiple_comments_tree):
        ...
        rule = CommentNormalizationRule()
        records = rule.apply(multiple_comments_tree)
    ...
    def test_empty_tree_returns_empty_list(self, empty_tree):
        """It returns an empty list when applied to an empty tree."""
        rule = CommentNormalizationRule()
        assert rule.apply(empty_tree) == []


class TestCommentNormalizationScopeAndContext:
    """Scope/context handling and edge case behavior."""

    def test_comment_in_function_scope(self, function_scope_comment_tree):
        """It normalizes comments inside a function scope."""
        comment_text = function_scope_comment_tree.children[0].text
        rule = CommentNormalizationRule()
        records = rule.apply(function_scope_comment_tree)
        assert len(records) == 1
        assert "new_val" in records[0].metadata
        assert records[0].metadata.get("new_val") != comment_text

    def test_comment_in_class_scope(self, class_scope_comment_tree):
        """It normalizes comments inside a class scope."""
        comment_text = class_scope_comment_tree.children[0].text
        rule = CommentNormalizationRule()
        records = rule.apply(class_scope_comment_tree)
        assert len(records) == 1
        assert "new_val" in records[0].metadata
        assert records[0].metadata.get("new_val") != comment_text

    def test_comment_in_loop_scope(self, loop_scope_comment_tree):
        """It normalizes comments inside a loop scope."""
        comment_text = loop_scope_comment_tree.children[0].text
        rule = CommentNormalizationRule()
        records = rule.apply(loop_scope_comment_tree)
        assert len(records) == 1
        assert "new_val" in records[0].metadata
        assert records[0].metadata.get("new_val") != comment_text

    def test_comment_in_condition_scope(self, condition_scope_comment_tree):
        """It normalizes comments inside a condition scope."""
        comment_text = condition_scope_comment_tree.children[0].text
        rule = CommentNormalizationRule()
        records = rule.apply(condition_scope_comment_tree)
        assert len(records) == 1
        assert "new_val" in records[0].metadata
        assert records[0].metadata.get("new_val") != comment_text

    def test_comment_after_terminal_statement(self, comment_after_terminal_tree):
        ...
        rule = CommentNormalizationRule()
        records = rule.apply(comment_after_terminal_tree)
        assert len(records) == 1
        assert "new_val" in records[0].metadata
        assert records[0].metadata.get("new_val") != comment_text
...
class TestCommentNormalizationFormatting:
    ...
    def test_comment_with_only_delimiters(self):
...
class TestCommentNormalizationErrorHandling:
    ...
    def test_comment_with_no_parent_raises(self, line_comment_tree):
        ...
        with pytest.raises(ValueError):
            rule.apply(line_comment_tree)

    def test_root_node_with_no_language_raises(self, line_comment_tree):
    ...
    def test_unsupported_delimiter_is_unchanged(self):
...
class TestCommentNormalizationMutationRecord:
    ...
    def test_mutation_record_content_block_comment(self, block_comment_tree):
        ...
        rule = CommentNormalizationRule()
        records = rule.apply(block_comment_tree)
        assert records
        record = records[0]
        assert isinstance(record, MutationRecord)
        assert record.node_id == comment.start_point
    ...
    def test_mutation_record_content_doc_comment(self):
        """It generates correct MutationRecord for normalized block comments."""
...
def test_rule_level_affects_replacement(line_comment_tree, level):
